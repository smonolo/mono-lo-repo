...
public class AdvancementService {
  ...
  // Live in-memory completion tracking per player (UUID -> (normalizedAdvancementId ->
  // timestampMillis))
  ...
  public synchronized void loadAdvancementsSync() {
    ...
    try {
      ...
      while (iterator.hasNext()) {
        try {
          ...
          String frame =
              display.frame() != null
                  ? display.frame().name()
                  : (existing != null ? existing.frame() : "TASK");
          ...
          String category =
              keyPath.contains("/") ? keyPath.split("/")[0] : adv.getKey().getNamespace();
          category = category.toLowerCase(Locale.ROOT);

          String categoryName = formatCategoryName(category);
          String parent =
              adv.getParent() != null
                  ? adv.getParent().getKey().toString()
                  : (existing != null ? existing.parent() : null);
          int criteriaCount =
              adv.getCriteria() != null
                  ? adv.getCriteria().size()
                  : (existing != null ? existing.criteriaCount() : 1);
      ...
      plugin
          .getLogger()
          .info(
              "AdvancementService refreshed with "
                  + registeredAdvancements.size()
                  + " total achievements.");
    } catch (Throwable e) {
  ...
  private String formatCategoryName(String category) {
    return switch (category.toLowerCase(Locale.ROOT)) {
      ...
      default -> {
        ...
        yield Character.toUpperCase(category.charAt(0)) + category.substring(1).replace('_', ' ');
      }
    };
  }
  ...
  /**
   * Recursively finds all directories named 'advancements' across the entire server. Result is
   * cached for 5 minutes to prevent continuous disk scanning.
   */
  ...
  private void scanForAdvancementFolders(
      File dir, int currentDepth, int maxDepth, Set<File> result, Set<String> visitedPaths) {
      ...
      if (subdirs != null) {
        for (File sub : subdirs) {
          ...
          if ("plugins".equalsIgnoreCase(name)
              || "libraries".equalsIgnoreCase(name)
              || "cache".equalsIgnoreCase(name)
  ...
  public List<File> findAllAdvancementFilesForPlayer(@NotNull UUID uuid) {
    ...
    for (File folder : getAllAdvancementFolders()) {
      ...
      File[] files =
          folder.listFiles((dir, name) -> name.toLowerCase(Locale.ROOT).endsWith(".json"));
      if (files != null) {
        for (File f : files) {
  ...
  public @NotNull JsonObject getPlayerAchievementsJson(@NotNull UUID uuid) {
    ...
    double percentage =
        totalCount > 0 ? Math.round((completedCount * 1000.0) / totalCount) / 10.0 : 0.0;
  ...
  private void loadBuiltInCatalog() {
    ...
    addCatalogItem(
        ...
        1);
    addCatalogItem(
        "minecraft:end/root", "The End", "Or the beginning?", "TASK", "end_stone", "end", null, 1);
    addCatalogItem(
    ...
    addCatalogItem(
        ...
        "Sniper Duel",
        "Kill a Skeleton with an arrow from more than 50 meters away",
        "CHALLENGE",
        "arrow",
        "adventure",
        "minecraft:adventure/shoot_arrow",
    ...
    addCatalogItem(
        ...
        "Light as a Rabbit",
        "Walk on powder snow... without sinking into it",
        "TASK",
        "leather_boots",
        "adventure",
        "minecraft:adventure/root",
    ...
    addCatalogItem(
        ...
        "Caves & Cliffs",
        "Free fall from the top of the world to the bottom",
        "TASK",
        "water_bucket",
        "adventure",
        "minecraft:adventure/root",
    ...
    addCatalogItem(
        ...
        "Smithing with Style",
        "Apply these smithing templates at least once",
        "CHALLENGE",
        "smithing_table",
        "adventure",
        "minecraft:adventure/trim_with_any_armor_pattern",
    ...
    addCatalogItem(
        ...
        "Isn't It Scute?",
        "Brush an Armadillo to obtain an Armadillo Scute",
        "TASK",
        "armadillo_scute",
        "adventure",
        "minecraft:adventure/salvage_sherd",
    ...
    addCatalogItem(
        ...
        "Who Needs Rockets?",
        "Launch yourself up at least 7 blocks using a Wind Charge",
        "TASK",
        "wind_charge",
        "adventure",
        "minecraft:adventure/root",
    ...
    addCatalogItem(
        ...
        "Lighten Up",
        "Un-oxidize a Copper Bulb using an Axe",
        "TASK",
        "copper_bulb",
        "adventure",
        "minecraft:adventure/root",
    ...
    addCatalogItem(
        ...
        "Under Lock and Key",
        "Unlock a Trial Spawner or Vault using a Trial Key",
        "TASK",
        "trial_key",
        "adventure",
        "minecraft:adventure/root",
    ...
    addCatalogItem(
        ...
        "Revaulting",
        "Unlock an Ominous Vault using an Ominous Trial Key",
        "GOAL",
        "ominous_trial_key",
        "adventure",
        "minecraft:adventure/under_lock_and_key",
    ...
    addCatalogItem(
        ...
        "Blowback",
        "Defeat a Breeze with a deflected Wind Charge",
        "CHALLENGE",
        "wind_charge",
        "adventure",
        "minecraft:adventure/who_needs_rockets",
        1);
    addCatalogItem(
        "minecraft:adventure/over_overkill",
        "Over-Overkill",
        "Deal 50 hearts of damage in a single hit using a Mace",
        "CHALLENGE",
        "mace",
        "adventure",
        "minecraft:adventure/root",
    ...
    addCatalogItem(
        ...
        1);
    addCatalogItem(
        "minecraft:adventure/walk_on_lava_with_boots",
        "Walk the Walk",
        "Walk safely over lava with Netherite boots",
        "TASK",
        "netherite_boots",
        "adventure",
        "minecraft:adventure/root",
        1);
    addCatalogItem(
        "minecraft:adventure/fall_with_mace",
        "Mace to Face",
        "Hit a target while falling using a Mace",
        "TASK",
        "mace",
        "adventure",
        "minecraft:adventure/over_overkill",
    ...
    addCatalogItem(
        ...
        1);
    addCatalogItem(
        "minecraft:adventure/ominous_trial_spawner",
        "A Real Challenge",
        "Trigger an Ominous Trial Spawner",
        "TASK",
        "trial_spawner",
        "adventure",
        "minecraft:adventure/under_lock_and_key",
    ...
    addCatalogItem(
        ...
        "Bee Our Guest",
        "Use a Campfire to collect Honey without aggravating the bees",
        "TASK",
        "honey_bottle",
        "husbandry",
        "minecraft:husbandry/root",
    ...
    addCatalogItem(
        ...
        1);
    addCatalogItem(
        "minecraft:husbandry/allay_drop_item_to_dump",
    ...
    addCatalogItem(
        ...
        1);
    addCatalogItem(
        "minecraft:husbandry/sniff_sniffer",
        "Smells Interesting",
    ...
    addCatalogItem(
        ...
        "Glow and Behold!",
        "Make the text of a Sign glow with a Glow Ink Sac",
        "TASK",
        "glow_ink_sac",
        "husbandry",
        "minecraft:husbandry/root",
    ...
    addCatalogItem(
        ...
        "Delivery Service",
        "Have an Allay deliver items to a player",
        "TASK",
        "cookie",
        "husbandry",
        "minecraft:husbandry/allay_drop_item_to_dump",
        1);
    addCatalogItem(
        "minecraft:husbandry/leash_all_horse_variants",
        "The Whole Herd",
        "Leash each horse variant",
        "TASK",
        "lead",
        "husbandry",
        "minecraft:husbandry/tame_an_animal",
        5);
    addCatalogItem(
        "minecraft:husbandry/brush_armadillo",
        "Isn't It Scute?",
        ...
        1);
    addCatalogItem(
        "minecraft:husbandry/feed_armadillo",
        "Armadillo Armor",
        "Feed an Armadillo with Spider Eyes",
        "TASK",
        "spider_eye",
        "husbandry",
        "minecraft:husbandry/brush_armadillo",
        1);
    addCatalogItem(
        "minecraft:husbandry/obtain_wolf_armor",
        "Suit Up Your Pup",
        "Equip Wolf Armor onto a tamed Wolf",
        "TASK",
        "wolf_armor",
        "husbandry",
        "minecraft:husbandry/brush_armadillo",
        1);
    addCatalogItem(
        "minecraft:husbandry/place_hanging_sign",
        "Hang in There",
        "Place a Hanging Sign",
        "TASK",
        "oak_hanging_sign",
        "husbandry",
        "minecraft:husbandry/make_a_sign_glow",
        1);
    addCatalogItem(
        "minecraft:husbandry/feed_allay",
        "Sweet Treat",
        "Give an Allay something sweet",
        "TASK",
        "sugar",
        "husbandry",
        "minecraft:husbandry/allay_drop_item_to_dump",
